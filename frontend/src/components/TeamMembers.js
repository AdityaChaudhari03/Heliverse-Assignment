...
const TeamMembers = () => {
  ...
  useEffect(() => {
    axios
      .get("https://heliverse-assignment-77x1.onrender.com/api/team")
      .then((response) => {
  ...
  const handleDetailsClick = async (memberId) => {
    try {
      const response = await axios.get(`https://heliverse-assignment-77x1.onrender.com/api/team/${memberId}`);
